import packageJson from '../package.json';

export const translations = {
    en: {
        // Global
        appName: 'DeviceHub',
        version: `Version: ${packageJson.version}`,
        loadingDots: 'Loading...',
        unknown: 'Unknown',
        cancel: 'Cancel',
        confirm: 'Confirm',
        delete: 'Delete',
        create: 'Create',
        on: 'On',
        off: 'Off',

        // App.tsx
        selectDeviceToInstall: 'Select a device to install',
        connectDevices: 'Connect Android devices to get started',

        // Settings
        settings: 'Settings',
        managePrefs: 'Manage application preferences',
        appearance: 'Appearance',
        appTheme: 'App Theme',
        selectTheme: 'Select your preferred interface style',
        adbConfig: 'ADB Configuration',
        customPath: 'Custom ADB Path',
        bundledAdb: 'Using bundled ADB',
        browse: 'Browse',
        leaveEmpty: 'Leave empty to use the bundled Android Debug Bridge',
        general: 'General',
        language: 'Language',
        changeLang: 'Change application language',
        notifications: 'Notifications',
        showNotif: 'Show system notifications for device events',
        about: 'About',
        aboutDesc: 'DeviceHub is a modern tool for managing Android devices.',
        checkUpdates: 'Check for updates',
        latestVersion: 'You are on the latest version!',
        logs: 'Logs',
        viewLogs: 'View Application Logs',
        reportIssue: 'Report an Issue',
        light: 'Light',
        dark: 'Dark',
        system: 'System',
        askBeforeSave: 'Ask where to save each time',
        askBeforeSaveDesc: 'Show a dialog to choose the file location for every capture',
        defaultSavePath: 'Default: Library/media',
        resetToDefaults: 'Reset to Defaults',
        resetConfirm: 'Are you sure you want to reset all settings to default? This action cannot be undone.',
        resetSuccess: 'Settings reset successfully',
        developedBy: 'Developed by',
        officialWebsite: 'Official Website',

        // Device List & Status
        noDevices: 'No devices connected',
        refresh: 'Refresh',
        authorizing: 'Authorizing...',
        unauthorized: 'Unauthorized',
        offline: 'Offline',
        connecting: 'Connecting...',
        searching: 'Searching for devices...',
        ready: 'Ready',
        installApk: 'Install APK',
        dropApkHere: 'Drop APK here',
        selectApk: 'Select APK',
        installing: 'Installing...',
        installSuccess: 'Installed successfully',
        installFailed: 'Installation failed',
        deviceConnected: 'Device connected',
        deviceDisconnected: 'Device disconnected',
        connectionError: 'Connection Error',
        connectedDevices: 'Connected Devices',

        // Hints
        hint1: 'Enable Developer Options on your device',
        hint2: 'Turn on USB Debugging',
        hint3: 'Connect device via USB cable',
        hint4: 'Accept the debugging prompt on device',

        // Requirement Checklist
        checking: 'Checking...',
        readyToInstall: 'Ready to install',
        issuesFound: 'issues found',
        issue: 'issue',
        issues: 'issues',

        // Requirement Names (IDs mapped to keys)
        req_developer_options: 'Developer Options',
        req_usb_debugging: 'USB Debugging',
        req_unknown_sources: 'Install Unknown Apps',
        req_auth: 'Device Authorization',
        req_usb_debug_security: 'USB Debugging (Security)',
        advancedActions: 'Advanced Actions',

        // Install Button
        btnInstall: 'Install',
        btnInstalling: 'Installing...',
        btnInstalled: 'Installed',
        btnFailed: 'Failed',
        apkInstalled: 'APK installed successfully',
        apkInstallFailed: 'Installation failed',

        // Device Actions / Overview
        deviceInfo: 'Device Info',
        model: 'Model',
        androidVersion: 'Android Version',
        sdkVersion: 'SDK Version',
        battery: 'Battery',
        charging: 'Charging',
        deviceId: 'Device ID',
        manufacturer: 'Manufacturer',
        screen: 'Screen',
        storage: 'Storage',
        free: 'free',
        ram: 'RAM',
        chipset: 'Chipset',
        build: 'Build',
        connection: 'Connection',
        wireless: 'Wireless',
        usb: 'USB',
        securityPatch: 'Security Patch',
        status: 'Status',

        // App Manager
        uninstallApp: 'Uninstall App',
        searchPackages: 'Search packages...',
        showSystemApps: 'Show system apps',
        noPackagesFound: 'No packages found',
        confirmUninstall: 'Are you sure you want to uninstall',
        uninstalling: 'Uninstalling...',
        uninstallSuccess: 'Uninstalled successfully',
        uninstallFailed: 'Uninstall failed',
        appUninstalled: 'App uninstalled',
        showingAllApps: 'Showing all apps',
        showingUserAppsOnly: 'Showing user apps only',
        packagesFound: 'packages found',
        allApps: 'All Apps',
        userAppsOnly: 'User Apps Only',
        uninstall: 'Uninstall',

        // Reboot Modal
        rebootDevice: 'Reboot Device',
        normalReboot: 'Normal Reboot',
        normalRebootDesc: 'Restart the device normally',
        recoveryMode: 'Recovery Mode',
        recoveryModeDesc: 'Boot into recovery mode',
        bootloaderMode: 'Bootloader Mode',
        bootloaderModeDesc: 'Boot into bootloader/fastboot',
        confirmReboot: 'The device will restart. Continue?',

        // Input Text Modal
        inputText: 'Input Text',
        textPlaceholder: 'Enter text to send to device...',
        sendText: 'Send',
        textSent: 'Text sent successfully',
        textFailed: 'Failed to send text',

        // File Manager
        fileTransfer: 'File Transfer',
        uploadFile: 'Upload',
        downloadFile: 'Download',
        deleteFile: 'Delete',
        createFolder: 'New Folder',
        emptyFolder: 'Empty folder',
        folderNamePlaceholder: 'Folder name...',
        confirmDelete: 'Are you sure you want to delete',
        folderName: 'Folder name...',
        internalStorage: 'Internal Storage',
        quickAccess: 'Quick Access',
        dcim: 'DCIM',
        pictures: 'Pictures',
        music: 'Music',
        movies: 'Movies',
        downloads: 'Downloads',
        root: 'Root',
        upload: 'Upload',
        newFolder: 'New Folder',
        goBack: 'Go Back',
        download: 'Download',
        fileDownloaded: 'File downloaded',
        downloadFailed: 'Download failed',
        deleted: 'Deleted',
        deleteFailed: 'Delete failed',
        folderCreated: 'Folder created',
        createFolderFailed: 'Create folder failed',
        selectFileUpload: 'Select file to upload',
        fileUploaded: 'File uploaded',
        uploadFailed: 'Upload failed',

        // Advanced/Tools Tab
        tabAdvanced: 'Advanced',
        selectFolder: 'Select Folder',
        wirelessAdb: 'Wireless ADB',
        wirelessAdbDesc: 'Connect devices over WiFi',
        logcat: 'Logcat',
        logcatDesc: 'View device logs',
        terminal: 'Terminal',
        terminalDesc: 'Execute shell commands',

        // Manual Connect
        connectViaIp: 'Connect via IP',
        offlineDevices: 'Offline Devices',
        removeDevice: 'Remove Device',
        refreshDevices: 'Refresh Devices',
        enterIpPort: 'Enter the IP address (and optional port) of the device. Ensure both devices are on the same network.',
        ipAddress: 'IP Address',
        ipPlaceholder: '192.168.1.x',
        btnConnect: 'Connect',
        btnConnecting: 'Connecting...',
        btnDisconnect: 'Disconnect',
        connectingTo: 'Connecting to',
        connectedTo: 'Connected to',
        failedToConnect: 'Failed to connect',
        acceptUsbDebug: 'Please accept USB debugging',
        deviceOffline: 'Device Offline',
        selectApkToEnable: 'Select an APK to enable',

        // Library / APK Manager
        library: 'LIBRARY',
        reloadFolder: 'Reload Folder',
        noValidApks: 'No valid APKs in folder',
        removeFromList: 'Remove from list',

        // Wireless ADB Modal (Special)
        step1EnableWireless: 'Step 1: Enable Wireless on USB Device',
        selectUsbDevice: 'Select USB device...',
        btnEnable: 'Enable',
        waitingForDevice: 'Waiting for device...',
        foundIp: 'Found IP',
        couldNotGetIp: 'Could not get IP',
        step2ConnectWirelessly: 'Step 2: Connect Wirelessly',
        enterIpAddress: 'Please enter device IP address',
        selectDeviceFirst: 'Please select a device first',

        // Screen Capture
        screenshotSaved: 'Screenshot saved',
        screenshotFailed: 'Screenshot failed',
        recordingSaved: 'Recording saved',
        recordingStopped: 'Recording stopped',
        failedToStopRecording: 'Failed to stop recording',
        recordingStarted: 'Recording started',
        failedToStartRecording: 'Failed to start recording',
        failedToGetScreenPreview: 'Failed to get screen preview',
        folderOpened: 'Folder opened',
        failedToOpenFolder: 'Failed to open folder',
        highPerfModeEnabled: 'High-performance mode enabled',
        failedToStartHighPerf: 'Failed to start high-performance mode',
        switchedToStandardMode: 'Switched to standard mode',
        failedToStopHighPerf: 'Failed to stop high-performance mode',
        failedToKey: 'Failed to send key event',
        capture: 'Capture',
        screenshot: 'Screenshot',
        stopRecording: 'Stop Recording',
        record: 'Record',
        configuration: 'Configuration',
        livePreview: 'Live Preview',
        enableTouch: 'Enable Touch',
        showFps: 'Show FPS',
        storageLocation: 'Captures saved to:',
        openFolder: 'Open Folder',
        quickActions: 'Quick Actions',
        back: 'Back',
        home: 'Home',
        recents: 'Recents',
        menu: 'Menu',
        volumeDown: 'Volume Down',
        volumeUp: 'Volume Up',
        mute: 'Mute',
        power: 'Power',
        brightnessDown: 'Brightness Down',
        brightnessUp: 'Brightness Up',
        playPause: 'Play/Pause',
        live: 'Live',
        noSignal: 'No Signal',
        recording: 'Recording',
        connectingStream: 'Connecting Stream...',
        fps: 'FPS',
        apk: 'APK',
        apkFiles: 'APK Files',
        removeApk: 'Remove APK',

        // AI API Keys
        aiApiKeys: 'AI API Keys',
        aiApiKeysDesc: 'Store API keys locally for AI-powered analysis features',
        aiKeysWarning: 'Keys are stored only in local storage and never sent anywhere except the respective AI provider.',
        openaiKey: 'OpenAI API Key',
        openaiKeyPlaceholder: 'sk-...',
        anthropicKey: 'Anthropic (Claude) API Key',
        anthropicKeyPlaceholder: 'sk-ant-...',
        geminiKey: 'Google Gemini API Key',
        geminiKeyPlaceholder: 'AIza...',
        aiKeySaved: 'API key saved',
        aiKeyCleared: 'API key cleared',

        // AI Log Analysis Modal
        aiLogAnalysis: 'AI Log Analysis',
        experimental: 'EXPERIMENTAL',
        aiUsingProvider: 'Using',
        aiNoKeyConfigured: 'No AI key configured',
        aiNoKeyDesc: 'Go to Settings → AI API Keys and add an OpenAI, Anthropic, or Gemini key to use this feature.',
        aiExperimentalWarning: 'This is an experimental feature. AI-generated analysis may be inaccurate. Do not use as a substitute for manual debugging. Logs are sent directly to the configured AI provider.',
        aiReadyToAnalyze: 'Ready to analyze',
        aiReadyDesc: 'characters of logs will be sent to',
        aiReadyDesc2: 'for analysis.',
        aiAnalyzing: 'Analyzing logs...',
        aiAnalyzingDesc: 'This may take a few seconds',
        aiAnalysisFailed: 'Analysis failed',
        aiAnalysisBy: 'Analysis by',
        aiChars: 'chars',
        aiAnalyze: 'Analyze',
        aiReanalyze: 'Re-analyze',
        copy: 'Copy',
        copied: 'Copied',
    },
    vi: {
        // Global
        appName: 'DeviceHub',
        version: `Phiên bản: ${packageJson.version}`,
        loadingDots: 'Đang tải...',
        unknown: 'Không xác định',
        cancel: 'Hủy',
        confirm: 'Xác nhận',
        delete: 'Xóa',
        create: 'Tạo',
        on: 'Bật',
        off: 'Tắt',

        // App.tsx
        selectDeviceToInstall: 'Chọn thiết bị để cài đặt',
        connectDevices: 'Kết nối thiết bị Android để bắt đầu',

        // Settings
        settings: 'Cài đặt',
        managePrefs: 'Quản lý tùy chọn ứng dụng',
        appearance: 'Giao diện',
        appTheme: 'Chủ đề',
        selectTheme: 'Chọn phong cách giao diện của bạn',
        adbConfig: 'Cấu hình ADB',
        customPath: 'Đường dẫn ADB tùy chỉnh',
        bundledAdb: 'Sử dụng ADB tích hợp',
        browse: 'Chọn',
        leaveEmpty: 'Để trống để sử dụng Android Debug Bridge tích hợp',
        general: 'Chung',
        language: 'Ngôn ngữ',
        changeLang: 'Thay đổi ngôn ngữ ứng dụng',
        notifications: 'Thông báo',
        showNotif: 'Hiển thị thông báo hệ thống cho các sự kiện thiết bị',
        about: 'Giới thiệu',
        aboutDesc: 'DeviceHub là công cụ hiện đại để quản lý thiết bị Android.',
        checkUpdates: 'Kiểm tra cập nhật',
        latestVersion: 'Bạn đang sử dụng phiên bản mới nhất!',
        logs: 'Nhật ký',
        viewLogs: 'Xem nhật ký ứng dụng',
        reportIssue: 'Báo cáo sự cố',
        light: 'Sáng',
        dark: 'Tối',
        system: 'Hệ thống',
        askBeforeSave: 'Hỏi nơi lưu mỗi lần',
        askBeforeSaveDesc: 'Hiện hộp thoại chọn vị trí lưu cho mỗi lần chụp/quay',
        defaultSavePath: 'Mặc định: Thư viện/media',
        resetToDefaults: 'Khôi phục mặc định',
        resetConfirm: 'Bạn có chắc chắn muốn khôi phục tất cả cài đặt về mặc định không? Thao tác này không thể hoàn tác.',
        resetSuccess: 'Đã khôi phục cài đặt gốc thành công',
        developedBy: 'Phát triển bởi',
        officialWebsite: 'Trang web chính thức',

        // Device List & Status
        noDevices: 'Không có thiết bị nào',
        refresh: 'Làm mới',
        authorizing: 'Đang xác thực...',
        unauthorized: 'Chưa xác thực',
        offline: 'Ngoại tuyến',
        connecting: 'Đang kết nối...',
        searching: 'Đang tìm kiếm thiết bị...',
        ready: 'Sẵn sàng',
        installApk: 'Cài đặt APK',
        dropApkHere: 'Thả file APK vào đây',
        selectApk: 'Chọn APK',
        installing: 'Đang cài đặt...',
        installSuccess: 'Cài đặt thành công',
        installFailed: 'Cài đặt thất bại',
        deviceConnected: 'Thiết bị đã kết nối',
        deviceDisconnected: 'Thiết bị đã ngắt kết nối',
        connectionError: 'Lỗi kết nối',
        connectedDevices: 'Thiết bị đã kết nối',

        // Hints
        hint1: 'Bật Tùy chọn nhà phát triển trên thiết bị',
        hint2: 'Bật Gỡ lỗi USB (USB Debugging)',
        hint3: 'Kết nối thiết bị bằng cáp USB',
        hint4: 'Chấp nhận lời nhắc gỡ lỗi trên thiết bị',

        // Requirement Checklist
        checking: 'Đang kiểm tra...',
        readyToInstall: 'Sẵn sàng cài đặt',
        issuesFound: 'vấn đề được tìm thấy',
        issue: 'vấn đề',
        issues: 'vấn đề',

        // Requirement Names
        req_developer_options: 'Tùy chọn nhà phát triển',
        req_usb_debugging: 'Gỡ lỗi USB',
        req_unknown_sources: 'Cài đặt ứng dụng không rõ nguồn gốc',
        req_auth: 'Xác thực thiết bị',
        req_usb_debug_security: 'Gỡ lỗi USB (Bảo mật)',
        advancedActions: 'Tính năng nâng cao',

        // Install Button
        btnInstall: 'Cài đặt',
        btnInstalling: 'Đang cài...',
        btnInstalled: 'Đã cài xong',
        btnFailed: 'Thất bại',
        apkInstalled: 'APK đã được cài đặt thành công',
        apkInstallFailed: 'Cài đặt thất bại',

        // Device Actions / Overview
        deviceInfo: 'Thông tin thiết bị',
        model: 'Model',
        androidVersion: 'Phiên bản Android',
        sdkVersion: 'Phiên bản SDK',
        battery: 'Pin',
        charging: 'Đang sạc',
        deviceId: 'ID thiết bị',
        manufacturer: 'Nhà sản xuất',
        screen: 'Màn hình',
        storage: 'Bộ nhớ',
        free: 'trống',
        ram: 'RAM',
        chipset: 'Chipset',
        build: 'Số hiệu bản dựng',
        connection: 'Kết nối',
        wireless: 'Không dây',
        usb: 'USB',
        securityPatch: 'Vá bảo mật',
        status: 'Trạng thái',

        // App Manager
        uninstallApp: 'Gỡ cài đặt',
        searchPackages: 'Tìm ứng dụng...',
        showSystemApps: 'Hiển thị ứng dụng hệ thống',
        noPackagesFound: 'Không tìm thấy ứng dụng',
        confirmUninstall: 'Bạn có chắc muốn gỡ cài đặt',
        uninstalling: 'Đang gỡ...',
        uninstallSuccess: 'Gỡ cài đặt thành công',
        uninstallFailed: 'Gỡ cài đặt thất bại',
        appUninstalled: 'Đã gỡ ứng dụng',
        showingAllApps: 'Đang hiện tất cả ứng dụng',
        showingUserAppsOnly: 'Chỉ hiện ứng dụng người dùng',
        packagesFound: 'gói được tìm thấy',
        allApps: 'Tất cả ứng dụng',
        userAppsOnly: 'Ứng dụng người dùng',
        uninstall: 'Gỡ cài đặt',

        // Reboot Modal
        rebootDevice: 'Khởi động lại',
        normalReboot: 'Khởi động lại thường',
        normalRebootDesc: 'Khởi động lại thiết bị bình thường',
        recoveryMode: 'Chế độ Recovery',
        recoveryModeDesc: 'Khởi động vào chế độ recovery',
        bootloaderMode: 'Chế độ Bootloader',
        bootloaderModeDesc: 'Khởi động vào bootloader/fastboot',
        confirmReboot: 'Thiết bị sẽ khởi động lại. Tiếp tục?',

        // Input Text Modal
        inputText: 'Nhập văn bản',
        textPlaceholder: 'Nhập văn bản để gửi đến thiết bị...',
        sendText: 'Gửi',
        textSent: 'Gửi văn bản thành công',
        textFailed: 'Gửi văn bản thất bại',

        // File Manager
        fileTransfer: 'Truyền tệp',
        uploadFile: 'Tải lên',
        downloadFile: 'Tải xuống',
        deleteFile: 'Xóa',
        createFolder: 'Thư mục mới',
        emptyFolder: 'Thư mục trống',
        folderNamePlaceholder: 'Tên thư mục...',
        confirmDelete: 'Bạn có chắc muốn xóa',
        folderName: 'Tên thư mục',
        internalStorage: 'Bộ nhớ trong',
        quickAccess: 'Truy cập nhanh',
        dcim: 'DCIM',
        pictures: 'Hình ảnh',
        music: 'Nhạc',
        movies: 'Phim',
        downloads: 'Tải xuống',
        root: 'Thư mục gốc (Root)',
        upload: 'Tải lên',
        newFolder: 'Thư mục mới',
        goBack: 'Quay lại',
        download: 'Tải xuống',
        fileDownloaded: 'Đã tải file',
        downloadFailed: 'Tải file thất bại',
        deleted: 'Đã xóa',
        deleteFailed: 'Xóa thất bại',
        folderCreated: 'Đã tạo thư mục',
        createFolderFailed: 'Tạo thư mục thất bại',
        selectFileUpload: 'Chọn tệp để tải lên',
        fileUploaded: 'Đã tải lên tệp',
        uploadFailed: 'Tải lên thất bại',

        // Advanced/Tools Tab
        tabAdvanced: 'Nâng cao',
        selectFolder: 'Chọn thư mục',
        wirelessAdb: 'Wireless ADB',
        wirelessAdbDesc: 'Kết nối thiết bị qua WiFi',
        logcat: 'Logcat',
        logcatDesc: 'Xem nhật ký thiết bị',
        terminal: 'Terminal',
        terminalDesc: 'Thực thi lệnh shell',

        // Manual Connect
        connectViaIp: 'Kết nối qua IP',
        offlineDevices: 'Thiết bị ngoại tuyến',
        removeDevice: 'Gỡ bỏ thiết bị',
        refreshDevices: 'Làm mới danh sách',
        enterIpPort: 'Nhập địa chỉ IP (và cổng tùy chọn). Đảm bảo cả hai thiết bị ở cùng mạng.',
        ipAddress: 'Địa chỉ IP',
        ipPlaceholder: '192.168.1.x',
        btnConnect: 'Kết nối',
        btnConnecting: 'Đang kết nối...',
        btnDisconnect: 'Ngắt kết nối',
        connectingTo: 'Đang kết nối tới',
        connectedTo: 'Đã kết nối tới',
        failedToConnect: 'Kết nối thất bại',
        acceptUsbDebug: 'Vui lòng chấp nhận gỡ lỗi USB',
        deviceOffline: 'Thiết bị ngoại tuyến',
        selectApkToEnable: 'Chọn APK để kích hoạt',

        // Library / APK Manager
        library: 'THƯ VIỆN',
        reloadFolder: 'Tải lại thư mục',
        noValidApks: 'Không có file APK nào trong thư mục',
        removeFromList: 'Xóa khỏi danh sách',

        // Wireless ADB Modal (Special)
        step1EnableWireless: 'Bước 1: Bật Wireless trên thiết bị USB',
        selectUsbDevice: 'Chọn thiết bị USB...',
        btnEnable: 'Bật',
        waitingForDevice: 'Đang chờ thiết bị...',
        foundIp: 'Đã tìm thấy IP',
        couldNotGetIp: 'Không thể lấy IP',
        step2ConnectWirelessly: 'Bước 2: Kết nối không dây',
        enterIpAddress: 'Vui lòng nhập địa chỉ IP thiết bị',
        selectDeviceFirst: 'Vui lòng chọn một thiết bị trước',

        // Screen Capture
        screenshotSaved: 'Đã lưu ảnh màn hình',
        screenshotFailed: 'Chụp màn hình thất bại',
        recordingSaved: 'Đã lưu video quay màn hình',
        recordingStopped: 'Đã dừng quay màn hình',
        failedToStopRecording: 'Dừng quay màn hình thất bại',
        recordingStarted: 'Đã bắt đầu quay màn hình',
        failedToStartRecording: 'Bắt đầu quay màn hình thất bại',
        failedToGetScreenPreview: 'Lỗi tải bản xem trước màn hình',
        folderOpened: 'Đã mở thư mục',
        failedToOpenFolder: 'Mở thư mục thất bại',
        highPerfModeEnabled: 'Đã bật chế độ hiệu năng cao',
        failedToStartHighPerf: 'Bật chế độ hiệu năng cao thất bại',
        switchedToStandardMode: 'Đã chuyển sang chế độ tiêu chuẩn',
        failedToStopHighPerf: 'Dừng chế độ hiệu năng cao thất bại',
        failedToKey: 'Gửi sự kiện phím thất bại',
        capture: 'Chụp & Quay',
        screenshot: 'Ảnh màn hình',
        stopRecording: 'Dừng quay',
        record: 'Quay màn hình',
        configuration: 'Cấu hình',
        livePreview: 'Bản xem trực tiếp',
        enableTouch: 'Cho phép điều khiển',
        showFps: 'Hiện FPS',
        storageLocation: 'Ảnh & video lưu tại:',
        openFolder: 'Mở thư mục',
        quickActions: 'Thao tác nhanh',
        back: 'Quay lại',
        home: 'Trang chủ',
        recents: 'Gần đây',
        menu: 'Menu',
        volumeDown: 'Giảm âm lượng',
        volumeUp: 'Tăng âm lượng',
        mute: 'Tắt tiếng',
        power: 'Nguồn',
        brightnessDown: 'Giảm độ sáng',
        brightnessUp: 'Tăng độ sáng',
        playPause: 'Phát/Tạm dừng',
        live: 'Trực tiếp',
        noSignal: 'Mất tín hiệu',
        recording: 'Đang quay',
        connectingStream: 'Đang kết nối luồng...',
        fps: 'FPS',
        apk: 'APK',
        apkFiles: 'Tệp APK',
        removeApk: 'Gỡ bỏ APK',

        // AI API Keys
        aiApiKeys: 'Khóa API AI',
        aiApiKeysDesc: 'Lưu trữ khóa API cho các tính năng phân tích bằng AI',
        aiKeysWarning: 'Khóa chỉ được lưu trong bộ nhớ cục bộ và không được gửi đi đâu ngoài nhà cung cấp AI tương ứng.',
        openaiKey: 'Khóa API OpenAI',
        openaiKeyPlaceholder: 'sk-...',
        anthropicKey: 'Khóa API Anthropic (Claude)',
        anthropicKeyPlaceholder: 'sk-ant-...',
        geminiKey: 'Khóa API Google Gemini',
        geminiKeyPlaceholder: 'AIza...',
        aiKeySaved: 'Đã lưu khóa API',
        aiKeyCleared: 'Đã xóa khóa API',

        // AI Log Analysis Modal
        aiLogAnalysis: 'Phân tích Log bằng AI',
        experimental: 'THỬ NGHIỆM',
        aiUsingProvider: 'Đang dùng',
        aiNoKeyConfigured: 'Chưa cấu hình khóa AI',
        aiNoKeyDesc: 'Vào Settings → AI API Keys và thêm khóa OpenAI, Anthropic hoặc Gemini để dùng tính năng này.',
        aiExperimentalWarning: 'Tính năng đang thử nghiệm (Experimental). Kết quả phân tích do AI tạo ra có thể không chính xác. Không dùng thay thế cho việc debug thủ công. Log được gửi trực tiếp đến nhà cung cấp AI đã cấu hình.',
        aiReadyToAnalyze: 'Sẵn sàng phân tích',
        aiReadyDesc: 'ký tự log sẽ được gửi đến',
        aiReadyDesc2: 'để phân tích.',
        aiAnalyzing: 'Đang phân tích log...',
        aiAnalyzingDesc: 'Quá trình này có thể mất vài giây',
        aiAnalysisFailed: 'Phân tích thất bại',
        aiAnalysisBy: 'Phân tích bởi',
        aiChars: 'ký tự',
        aiAnalyze: 'Phân tích',
        aiReanalyze: 'Phân tích lại',
        copy: 'Sao chép',
        copied: 'Đã sao chép',
    },
    fr: {
        // Global
        appName: 'DeviceHub',
        version: `Version : ${packageJson.version}`,
        loadingDots: 'Chargement...',
        unknown: 'Inconnu',
        cancel: 'Annuler',
        confirm: 'Confirmer',
        delete: 'Supprimer',
        create: 'Créer',
        on: 'Activé',
        off: 'Désactivé',

        // App.tsx
        selectDeviceToInstall: 'Sélectionnez un appareil pour installer',
        connectDevices: 'Connectez des appareils Android pour commencer',

        // Settings
        settings: 'Paramètres',
        managePrefs: 'Gérer les préférences de l\'application',
        appearance: 'Apparence',
        appTheme: 'Thème de l\'application',
        selectTheme: 'Choisissez votre style d\'interface préféré',
        adbConfig: 'Configuration ADB',
        customPath: 'Chemin ADB personnalisé',
        bundledAdb: 'Utilisation de l\'ADB intégré',
        browse: 'Parcourir',
        leaveEmpty: 'Laissez vide pour utiliser Android Debug Bridge intégré',
        general: 'Général',
        language: 'Langue',
        changeLang: 'Changer la langue de l\'application',
        notifications: 'Notifications',
        showNotif: 'Afficher les notifications système pour les événements d\'appareil',
        about: 'À propos',
        aboutDesc: 'DeviceHub est un outil moderne pour gérer les appareils Android.',
        checkUpdates: 'Vérifier les mises à jour',
        latestVersion: 'Vous utilisez la dernière version !',
        logs: 'Journaux',
        viewLogs: 'Voir les journaux de l\'application',
        reportIssue: 'Signaler un problème',
        light: 'Clair',
        dark: 'Sombre',
        system: 'Système',
        askBeforeSave: 'Demander où enregistrer à chaque fois',
        askBeforeSaveDesc: 'Afficher une boîte de dialogue pour choisir l\'emplacement du fichier pour chaque capture',
        defaultSavePath: 'Par défaut : Bibliothèque/media',
        resetToDefaults: 'Réinitialiser par défaut',
        resetConfirm: 'Êtes-vous sûr de vouloir réinitialiser tous les paramètres par défaut ? Cette action ne peut pas être annulée.',
        resetSuccess: 'Paramètres réinitialisés avec succès',
        developedBy: 'Développé par',
        officialWebsite: 'Site officiel',

        // Device List & Status
        noDevices: 'Aucun appareil connecté',
        refresh: 'Actualiser',
        authorizing: 'Autorisation...',
        unauthorized: 'Non autorisé',
        offline: 'Hors ligne',
        connecting: 'Connexion...',
        searching: 'Recherche d\'appareils...',
        ready: 'Prêt',
        installApk: 'Installer APK',
        dropApkHere: 'Déposez l\'APK ici',
        selectApk: 'Sélectionner APK',
        installing: 'Installation...',
        installSuccess: 'Installation réussie',
        installFailed: 'Échec de l\'installation',
        deviceConnected: 'Appareil connecté',
        deviceDisconnected: 'Appareil déconnecté',
        connectionError: 'Erreur de connexion',
        connectedDevices: 'Appareils connectés',

        // Hints
        hint1: 'Activez les options pour développeurs sur votre appareil',
        hint2: 'Activez le débogage USB',
        hint3: 'Connectez l\'appareil via un câble USB',
        hint4: 'Acceptez l\'invite de débogage sur l\'appareil',

        // Requirement Checklist
        checking: 'Vérification...',
        readyToInstall: 'Prêt à installer',
        issuesFound: 'problèmes trouvés',
        issue: 'problème',
        issues: 'problèmes',

        // Requirement Names
        req_developer_options: 'Options pour développeurs',
        req_usb_debugging: 'Débogage USB',
        req_unknown_sources: 'Installer des applications inconnues',
        req_auth: 'Autorisation de l\'appareil',
        req_usb_debug_security: 'Débogage USB (Sécurité)',
        advancedActions: 'Actions avancées',

        // Install Button
        btnInstall: 'Installer',
        btnInstalling: 'Installation...',
        btnInstalled: 'Installé',
        btnFailed: 'Échec',
        apkInstalled: 'APK installé avec succès',
        apkInstallFailed: 'Échec de l\'installation',

        // Device Actions / Overview
        deviceInfo: 'Infos appareil',
        model: 'Modèle',
        androidVersion: 'Version Android',
        sdkVersion: 'Version SDK',
        battery: 'Batterie',
        charging: 'En charge',
        deviceId: 'ID appareil',
        manufacturer: 'Fabricant',
        screen: 'Écran',
        storage: 'Stockage',
        free: 'libre',
        ram: 'RAM',
        chipset: 'Chipset',
        build: 'Version',
        connection: 'Connexion',
        wireless: 'Sans fil',
        usb: 'USB',
        securityPatch: 'Correctif de sécurité',
        status: 'Statut',

        // App Manager
        uninstallApp: 'Désinstaller l\'application',
        searchPackages: 'Rechercher des packages...',
        showSystemApps: 'Afficher les applications système',
        noPackagesFound: 'Aucun package trouvé',
        confirmUninstall: 'Êtes-vous sûr de vouloir désinstaller',
        uninstalling: 'Désinstallation...',
        uninstallSuccess: 'Désinstallation réussie',
        uninstallFailed: 'Échec de la désinstallation',
        appUninstalled: 'Application désinstallée',
        showingAllApps: 'Affichage de toutes les applications',
        showingUserAppsOnly: 'Affichage des applications utilisateur uniquement',
        packagesFound: 'packages trouvés',
        allApps: 'Toutes les applications',
        userAppsOnly: 'Applications utilisateur',
        uninstall: 'Désinstaller',

        // Reboot Modal
        rebootDevice: 'Redémarrer l\'appareil',
        normalReboot: 'Redémarrage normal',
        normalRebootDesc: 'Redémarrer l\'appareil normalement',
        recoveryMode: 'Mode Recovery',
        recoveryModeDesc: 'Démarrer en mode recovery',
        bootloaderMode: 'Mode Bootloader',
        bootloaderModeDesc: 'Démarrer en mode bootloader/fastboot',
        confirmReboot: 'L\'appareil va redémarrer. Continuer ?',

        // Input Text Modal
        inputText: 'Saisir du texte',
        textPlaceholder: 'Entrez le texte à envoyer à l\'appareil...',
        sendText: 'Envoyer',
        textSent: 'Texte envoyé avec succès',
        textFailed: 'Échec de l\'envoi du texte',

        // File Manager
        fileTransfer: 'Transfert de fichiers',
        uploadFile: 'Téléverser',
        downloadFile: 'Télécharger',
        deleteFile: 'Supprimer',
        createFolder: 'Nouveau dossier',
        emptyFolder: 'Dossier vide',
        folderNamePlaceholder: 'Nom du dossier...',
        confirmDelete: 'Êtes-vous sûr de vouloir supprimer',
        folderName: 'Nom du dossier...',
        internalStorage: 'Stockage interne',
        quickAccess: 'Accès rapide',
        dcim: 'DCIM',
        pictures: 'Images',
        music: 'Musique',
        movies: 'Films',
        downloads: 'Téléchargements',
        root: 'Racine',
        upload: 'Téléverser',
        newFolder: 'Nouveau dossier',
        goBack: 'Retour',
        download: 'Télécharger',
        fileDownloaded: 'Fichier téléchargé',
        downloadFailed: 'Échec du téléchargement',
        deleted: 'Supprimé',
        deleteFailed: 'Échec de la suppression',
        folderCreated: 'Dossier créé',
        createFolderFailed: 'Échec de la création du dossier',
        selectFileUpload: 'Sélectionner un fichier à téléverser',
        fileUploaded: 'Fichier téléversé',
        uploadFailed: 'Échec du téléversement',

        // Advanced/Tools Tab
        tabAdvanced: 'Avancé',
        selectFolder: 'Sélectionner un dossier',
        wirelessAdb: 'ADB sans fil',
        wirelessAdbDesc: 'Connecter des appareils via WiFi',
        logcat: 'Logcat',
        logcatDesc: 'Voir les journaux de l\'appareil',
        terminal: 'Terminal',
        terminalDesc: 'Exécuter des commandes shell',

        // Manual Connect
        connectViaIp: 'Connexion via IP',
        offlineDevices: 'Appareils hors ligne',
        removeDevice: 'Supprimer l\'appareil',
        refreshDevices: 'Actualiser les appareils',
        enterIpPort: 'Entrez l\'adresse IP (et le port optionnel) de l\'appareil. Assurez-vous que les deux appareils sont sur le même réseau.',
        ipAddress: 'Adresse IP',
        ipPlaceholder: '192.168.1.x',
        btnConnect: 'Connecter',
        btnConnecting: 'Connexion...',
        btnDisconnect: 'Déconnecter',
        connectingTo: 'Connexion à',
        connectedTo: 'Connecté à',
        failedToConnect: 'Échec de la connexion',
        acceptUsbDebug: 'Veuillez accepter le débogage USB',
        deviceOffline: 'Appareil hors ligne',
        selectApkToEnable: 'Sélectionnez un APK pour activer',

        // Library / APK Manager
        library: 'BIBLIOTHÈQUE',
        reloadFolder: 'Recharger le dossier',
        noValidApks: 'Aucun APK valide dans le dossier',
        removeFromList: 'Supprimer de la liste',

        // Wireless ADB Modal (Special)
        step1EnableWireless: 'Étape 1 : Activer le sans fil sur l\'appareil USB',
        selectUsbDevice: 'Sélectionner l\'appareil USB...',
        btnEnable: 'Activer',
        waitingForDevice: 'En attente de l\'appareil...',
        foundIp: 'IP trouvée',
        couldNotGetIp: 'Impossible d\'obtenir l\'IP',
        step2ConnectWirelessly: 'Étape 2 : Connexion sans fil',
        enterIpAddress: 'Veuillez entrer l\'adresse IP de l\'appareil',
        selectDeviceFirst: 'Veuillez d\'abord sélectionner un appareil',

        // Screen Capture
        screenshotSaved: 'Capture d\'écran enregistrée',
        screenshotFailed: 'Échec de la capture d\'écran',
        recordingSaved: 'Enregistrement sauvegardé',
        recordingStopped: 'Enregistrement arrêté',
        failedToStopRecording: 'Échec de l\'arrêt de l\'enregistrement',
        recordingStarted: 'Enregistrement démarré',
        failedToStartRecording: 'Échec du démarrage de l\'enregistrement',
        failedToGetScreenPreview: 'Échec de l\'aperçu de l\'écran',
        folderOpened: 'Dossier ouvert',
        failedToOpenFolder: 'Échec de l\'ouverture du dossier',
        highPerfModeEnabled: 'Mode haute performance activé',
        failedToStartHighPerf: 'Échec du démarrage du mode haute performance',
        switchedToStandardMode: 'Passé en mode standard',
        failedToStopHighPerf: 'Échec de l\'arrêt du mode haute performance',
        failedToKey: 'Échec de l\'envoi de l\'événement clavier',
        capture: 'Capture',
        screenshot: 'Capture d\'écran',
        stopRecording: 'Arrêter l\'enregistrement',
        record: 'Enregistrer',
        configuration: 'Configuration',
        livePreview: 'Aperçu en direct',
        enableTouch: 'Activer le tactile',
        showFps: 'Afficher FPS',
        storageLocation: 'Captures enregistrées dans :',
        openFolder: 'Ouvrir le dossier',
        quickActions: 'Actions rapides',
        back: 'Retour',
        home: 'Accueil',
        recents: 'Récents',
        menu: 'Menu',
        volumeDown: 'Volume -',
        volumeUp: 'Volume +',
        mute: 'Muet',
        power: 'Alimentation',
        brightnessDown: 'Luminosité -',
        brightnessUp: 'Luminosité +',
        playPause: 'Lecture/Pause',
        live: 'En direct',
        noSignal: 'Pas de signal',
        recording: 'Enregistrement',
        connectingStream: 'Connexion au flux...',
        fps: 'IPS',
        apk: 'APK',
        apkFiles: 'Fichiers APK',
        removeApk: 'Supprimer l\'APK',

        // AI API Keys
        aiApiKeys: 'Clés API IA',
        aiApiKeysDesc: 'Stocker les clés API localement pour les fonctionnalités d\'analyse IA',
        aiKeysWarning: 'Les clés sont stockées uniquement dans le stockage local et ne sont jamais envoyées ailleurs qu\'au fournisseur IA respectif.',
        openaiKey: 'Clé API OpenAI',
        openaiKeyPlaceholder: 'sk-...',
        anthropicKey: 'Clé API Anthropic (Claude)',
        anthropicKeyPlaceholder: 'sk-ant-...',
        geminiKey: 'Clé API Google Gemini',
        geminiKeyPlaceholder: 'AIza...',
        aiKeySaved: 'Clé API enregistrée',
        aiKeyCleared: 'Clé API effacée',

        // AI Log Analysis Modal
        aiLogAnalysis: 'Analyse des logs par IA',
        experimental: 'EXPÉRIMENTAL',
        aiUsingProvider: 'Utilisation de',
        aiNoKeyConfigured: 'Aucune clé IA configurée',
        aiNoKeyDesc: 'Allez dans Paramètres → Clés API IA et ajoutez une clé OpenAI, Anthropic ou Gemini pour utiliser cette fonctionnalité.',
        aiExperimentalWarning: 'Fonctionnalité expérimentale. L\'analyse générée par l\'IA peut être inexacte. Ne pas utiliser comme substitut au débogage manuel. Les logs sont envoyés directement au fournisseur IA configuré.',
        aiReadyToAnalyze: 'Prêt à analyser',
        aiReadyDesc: 'caractères de logs seront envoyés à',
        aiReadyDesc2: 'pour analyse.',
        aiAnalyzing: 'Analyse des logs...',
        aiAnalyzingDesc: 'Cela peut prendre quelques secondes',
        aiAnalysisFailed: 'Échec de l\'analyse',
        aiAnalysisBy: 'Analyse par',
        aiChars: 'caractères',
        aiAnalyze: 'Analyser',
        aiReanalyze: 'Réanalyser',
        copy: 'Copier',
        copied: 'Copié',
    },
};

export type Language = keyof typeof translations;
